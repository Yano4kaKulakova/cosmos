...
<table>
  ...
  <?php
    $sql = mysqli_query($link, 'SELECT * FROM `services`');
    while ($result = mysqli_fetch_array($sql)) {
      echo '<tr>'.
          "<td>{$result['name']}</td>".
          "<td>{$result['price']} ₽</td>".
          "<td><a href='edit.php?id={$result['id']}'>Изменить</a></td>".
  ...
  ?>
    ...
    <input type="text" name="name" placeholder="Введите наименование услуги" size="51">
